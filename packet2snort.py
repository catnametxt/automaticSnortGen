# Program which studies a specified packet from a specified PCAP, generating a snort rule from what it sees
...
try:
    from scapy.all import IP, TCP, DNS, DNSQR, DNSRR, UDP, ICMP
except ImportError:
	sys.stderr.write("ERROR: You must have scapy installed.\n")
	sys.stderr.write("You can install it by running: sudo pip install -U 'scapy>=2.3,<2.4'")
	exit(1)
...
# Check if DNS is present in the packet				
				if DNS in singlepacket:
					...
					if DNSRR in singlepacket:
						...
						print(("alert udp any 53 -> $HOME_NET any (msg: \"Suspicious DNS reply for {0} with address {1} detected!\"; content:\"|00 01 00 01|\"; content:\"|00 04".format(hostname, hostaddr)), end=' ')
						addrsplit = hostaddr.split('.')
						for addr in addrsplit:
							hexaddr = format(int(addr), '02x')
							print("\b",hexaddr.upper(), end=' ')
						print("\b|\"; distance:4; reference:Packet2Snort; classtype:trojan-activity; sid:xxxx; rev:1;)")
					else:
						print(("alert udp $HOME_NET any -> any 53 (msg: \"Suspicious DNS request for {0} detected!\"; content:\"|01 00 00 01 00 00 00 00 00 00|\"; depth:10; offset:2; content:\"".format(hostname)), end=' ')
						...
						for word in dnsplit:
							if word != '':
								...
								upper = hexa.upper()
								print(("\b|{0}|{1}".format(upper, word)), end=' ')
...
# When no argument is give, return the usage function		
		else:
			usage()
